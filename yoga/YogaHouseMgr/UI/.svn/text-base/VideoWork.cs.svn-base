...
namespace UI
{
    public class VideoWork
    {
        ...
        public void Stop()
        {
            int limit = 0;
            bool result = false;
            while (!(result =SendMessage(hWndC, WM_CAP_DRIVER_DISCONNECT, 0, 0)))
            {
                if (limit++ > 20) break;
            }
            if (result)
        ...
        public bool GrabImage(string path)
        {
            ...
            while (!(result = SendMessage(hWndC, WM_CAP_SAVEDIB, 0, hBmp.ToInt32())))
            {
                if(limit++>20)break;
            }
            if(!result)
            MessageBox.Show("摄像头发生异常，截图失败");
            return result;
        }
    }
